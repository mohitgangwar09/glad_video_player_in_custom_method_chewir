...
class NegotiatePriceOTP extends StatefulWidget {
  final User projectData;
  final String livestockId;
  final String userId;
  final String negotiatedPrice;

  const NegotiatePriceOTP({super.key,required this.projectData, required this.livestockId, required this.userId, required this.negotiatedPrice});

  @override
  State<NegotiatePriceOTP> createState() => _NegotiatePriceOTPState();
}

class _NegotiatePriceOTPState extends State<NegotiatePriceOTP> {
        ...
        customButton('Send OTP', fontColor: 0xffFFFFFF,
            onTap: istClickOnSendOtp ==""?() {
              ...
              BlocProvider.of<ProjectCubit>(context).sendProjectStatusOtpApi(context,
                  widget.projectData!.mobile.toString()
              );
      ...
      child: Stack(
        children: [
          PinCodeTextField(
            ...
            onChanged: (value) {
              if(value.length==4){
                  BlocProvider.of<ProjectCubit>(context).verifyNegotiatePriceApi(context, value.toString(),
                      widget.projectData.id.toString(),widget.livestockId, widget.negotiatedPrice, widget.userId);
